...
import { NumTextField, DateTimeField, WidgetBox, PillButton } from './ReusableComponents';
...
function TimeCalculator() {
  ...
  const [disp, setDisp] = useState({depTimeStr: "", sleepTimeStr: "", wakeTimeStr: ""});
  ...
  const calc = () => {
    ...
    if (isEnterDur) {
      ...
      depTimeCalc = sub(arrTime, { hours: parseInt(travelDurSplit[0]), minutes: parseInt(travelDurSplit[1]) });
    } else {
      if (!depTime) return;
    ...
    wakeTime = depTimeCalc && prepDur && prepDurSplit.length === 2 
      ? sub(depTimeCalc, { hours: parseInt(prepDurSplit[0]), minutes: parseInt(prepDurSplit[1]) })
      : null;
    sleepTime = wakeTime && sleepDur && sleepDurSplit.length === 2
      ? sub(wakeTime, { hours: parseInt(sleepDurSplit[0]), minutes: parseInt(sleepDurSplit[1]) })
      : null;

    let depTimeStr = !depTimeCalc ? "" : format(depTimeCalc, "dd/MM/yyyy, hh:mm a");
    let sleepTimeStr = !sleepTime ? "" : format(sleepTime, "dd/MM/yyyy, hh:mm a");
    ...
    console.log("depTime", depTimeStr, "sleepTime", sleepTimeStr, "wakeTime", wakeTimeStr);
  };
  ...
  return (
    <ThemeProvider theme={theme}>
      <WidgetBox>
        <Typography variant="h3" marginBottom={"0.5rem"} color="primaryText">Time Calculator</Typography>
        ...
        <Box>
          ...
          <Button
            ...
            sx={{ fontSize: "0.8rem" }}
          >
            {isEnterDur ? "Enter departure time instead" : "Enter commute duration instead"}
          </Button>            
        ...
        {isDisp ? (
          <Box>
            <Typography color="primaryText">💤 Sleep by: {disp.sleepTimeStr}</Typography>
            <Typography color="primaryText">⏰ Wake by: {disp.wakeTimeStr}</Typography>
            <Typography color="primaryText">🚌 Depart by: {disp.depTimeStr}</Typography>
          </Box>
        ) : (
          <Box></Box>
        )}
